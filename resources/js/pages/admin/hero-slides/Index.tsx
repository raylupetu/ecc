...
export default function Index({ slides }: Props) {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Êtes-vous sûr de vouloir supprimer cette bannière ?')) {
            destroy(route('hero-slides.destroy', id));
        }
    };

    return (
        <AuthenticatedLayout>
            <Head title="Bannières Hero" />

            <div className="space-y-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 tracking-tight">Bannières Hero</h1>
                        <p className="text-gray-500 font-medium">Gérez le contenu défilant de la page d'accueil.</p>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 border-b border-gray-100">
                                    <th className="px-8 py-6 text-sm font-black text-gray-400 uppercase tracking-widest">Aperçu</th>
                                    <th className="px-8 py-6 text-sm font-black text-gray-400 uppercase tracking-widest">Titre (FR/EN)</th>
                                    <th className="px-8 py-6 text-sm font-black text-gray-400 uppercase tracking-widest text-center">Ordre</th>
                                ...
                                {slides.map((slide, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-8 py-6">
                                            <span className="font-bold text-gray-900 block line-clamp-1">{slide.title_fr}</span>
                                            <span className="text-gray-400 text-xs font-medium line-clamp-1">{slide.title_en}</span>
                                        </td>
                                ...
                                {slides.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="px-8 py-20 text-center">
                                            ...
                                            <p className="text-gray-400 font-medium italic">Aucune bannière trouvée.</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
