...
export default function Edit({ verse }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        text_fr: verse.text_fr,
        text_en: verse.text_en,
        reference_fr: verse.reference_fr,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('bible-verses.update', verse.id));
    };

    return (
        <AuthenticatedLayout>
            <Head title={`Modifier Verset ${verse.id}`} />
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 tracking-tight">Modifier Verset</h1>
                        <p className="text-gray-500 font-medium">Référence : {verse.reference_fr}</p>
                    </div>
                </div>
